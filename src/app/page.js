...
import { connectDB } from "@/lib/mongoClient";  // Import the connectDB function
...
export default async function Page() {
  ...
  // Connect to the database using the connectDB helper
  const db = await connectDB();
  const animeCollection = db.collection("animoon-schedule");

  // Fetch anime documents from MongoDB
  let animeDocs = await animeCollection.find({}).toArray();
  animeDocs = JSON.parse(JSON.stringify(animeDocs)); // Ensure proper serialization of documents
